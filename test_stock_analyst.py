...
class StockAnalystTests(unittest.TestCase):
    ...
    def test_dashboard_html_exposes_app_controls(self):
        ...
        self.assertIn("panelOrder = ['watchlist', 'market', 'research']", document)
    ...
    def test_report_header_omits_old_logo(self):
        ...
        self.assertIn("List is subject to change", document)
        self.assertIn("Do not open any listed positions until specified by your ATLAS agent", document)
        self.assertIn("Last edited:", document)
        self.assertIn("data-generated-at=", document)
        self.assertIn("grid-template-rows .34s", document)
        self.assertIn("updateLastEdited", document)
