...
class StockAnalystTests(unittest.TestCase):
    ...
    def test_dashboard_html_exposes_app_controls(self):
        ...
        self.assertIn(">ATLAS watchlist</button>", document)
        self.assertIn(">personal watchlist</button>", document)
        self.assertIn(">favorites</button>", document)
        self.assertIn('data-watchlist-panel="atlas"', document)
        self.assertIn('data-watchlist-content="personal"', document)
        self.assertIn("showWatchlistPanel", document)
        self.assertIn(">Watchlists</span>", document)
        self.assertIn(">Journal</span>", document)
        self.assertIn(">Research</span>", document)
        self.assertIn('"Helvetica Neue", Helvetica, Arial, sans-serif', document)
        self.assertIn("max-width: 607px", document)
        self.assertIn("padding: 86px 27px 138px", document)
        self.assertIn("padding: 86px 20px 138px", document)
        self.assertIn("width: min(553px, calc(100vw - 54px))", document)
        self.assertIn("bottom: 50px", document)
        self.assertIn("grid-template-columns: repeat(3, 1fr)", document)
        self.assertNotIn("touchstart", document)
        self.assertNotIn("touchend", document)
    ...
    def test_report_header_omits_old_logo(self):
        ...
        self.assertIn("padding: 0 0 240px", document)
        self.assertIn("max-width: none", document)
        self.assertIn("min-height: 118px", document)
        self.assertIn("grid-template-columns: minmax(0, 1fr) 116px 54px", document)
        self.assertIn("scrollIntoView", document)
        self.assertIn("font-weight: 600", document)
